...
def case_valid_fixture_names_commands_and_inputs() -> None:
    ...
    assert [g["command"] for g in gates] == [
        ["rumdl", "fmt", "--check", "."],
    ...
    return read(
        {
            ...
            "gates": [
                {"name": "", "command": []},
                valid_gate(name="twin"),
                valid_gate(name="twin", timeout=0, surprise=1),
            ],
        }
    )
    ...
    result = read(
        {
            "version": 1,
            "gates": [
                valid_gate(name="a", depends_on=["b"]),
                valid_gate(name="b", depends_on=["c"]),
    ...
    result = read(
        {
            "version": 1,
            "gates": [
                valid_gate(name="base"),
                valid_gate(name="left", depends_on=["base"]),
...
def case_missing_repository_exits_2() -> None:
    with tempfile.TemporaryDirectory() as directory:
        absent = str(Path(directory) / "absent")
        result = subprocess.run(
            [sys.executable, str(SCRIPT), "--repo-root", absent],
            capture_output=True,
            text=True,
            check=False,
        )
...
for name, fn in [
    ...
    ("a repository path that does not exist exits 2", case_missing_repository_exits_2),
]:
    check(name, fn)
