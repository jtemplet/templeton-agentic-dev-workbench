...
`publish-plugin` must still consume `SHIP_DONE <sha>` or `SHIP_BLOCKED <reason>`. The reported
...
| `skills/publish-plugin/SKILL.md` | Existing consumer of ship's machine line | Verify compatibility |
...
| [0003](../adr/0003-a-push-to-main-is-already-published.md) | Pushing the default branch distributes the plugin | Complete verification before publication; leave numbering to publish-plugin |
...
25. `publish-plugin` consumes the unchanged machine line successfully, including every existing
    blocked category.
