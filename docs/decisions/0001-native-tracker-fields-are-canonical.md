...
`plan-to-beads/SKILL.md:486` instructs: "Pass the Why, How, Done when, type-specific sections, AND Estimated size body to `bd create` via `-d`; do not strip any section to save a line." Its command templates put every section into the `--description` body as markdown headings.

`bead-audit/SKILL.md:92-99` states the opposite for trackers with first-class fields. `bd` has native `design`, `notes`, and `acceptance_criteria` fields, and the skill maps How to `design`, Done when to `notes`, and Acceptance Criteria to `acceptance_criteria`. It then rules that "a section's content duplicated into the body when a native field exists for it is `structure: variant`," an auto-fixable reformat.
...
- **Cons:** Permanently forfeits `bd`'s structured fields. Any tooling reading `acceptance_criteria` sees an empty field on every bead. The tracker's schema becomes decorative.
...
- `bd list --json` returns each section in its own field, so a refinement loop or dashboard reads structured data instead of parsing markdown headings.
