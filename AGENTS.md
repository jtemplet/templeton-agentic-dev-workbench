...
- `bead-audit` - Audit one or more bead issue bodies against the Marr audit, size audit, type-specific section audit, and a **grounding audit against the main branch**; tracker-agnostic (accepts pasted text, files, or any CLI output) but honors native tracker fields (e.g. `br`'s acceptance_criteria/design/notes); separates three independent verdicts, content (is the substance there?), structure (is it under the canonical heading?), and grounding (is the bead still true of today's code?), so format-only issues are an auto-fixable REFORMAT rather than a FAIL, and a well-written bead whose target code moved is `drifted` rather than under-specified; exempts epics and operational beads from the size band, self-verifies each drafted fix so it re-passes, gates write-back with an `applyable` flag so placeholder-bearing drafts never reach the tracker, and supports a JSON output mode for backlog-scale grooming. Emits an optional weighted scorecard (0-100, banded Poor/Weak/Adequate/Great/Excellent) derived from the verdicts and capped so the band can never outrank the pass/fail verdict or the grounding verdict, for ranking a backlog or gating on a target band; the rubric ships with a fixture regression suite under `references/fixtures/`

  **Grounding specifics.** Only *current-state* claims are checkable: a bead's Why, How, and Steps to Reproduce describe the world as it is, while its Done when and Acceptance Criteria describe the world after the work and are supposed to be false while the bead is open. Checking those for drift would fail every open bead, so they are run the other way instead: an end state that **already holds** makes the bead `satisfied` (done or obsolete, propose closure). Claims are checked against `origin/main` with `git show` and `git grep`, never the working tree, which on a feature branch already contains the change the bead asks for and would report unmerged work as `satisfied`. The verdict is one of `grounded` / `drifted` / `satisfied` / `ungroundable`, and `ungroundable` (no repo, wrong repo, nothing checkable) is reported explicitly so an unchecked backlog is never mistaken for a verified one. Grounding contributes no points; it applies a band ceiling (`drifted` caps at Adequate, `satisfied` at Weak) and forces `applyable: false`, because a false premise has two opposite resolutions ("the bead is stale" and "the code regressed") that the audit cannot distinguish. The four checks (existence, pattern, stack, behavior) are the same ones `plan-review` uses, deliberately sharing one vocabulary. **The ceilings themselves are unpinned by any fixture**: the eight fixtures name no *repository of record* (they sit inside this repo but describe a fictional codebase), so all eight are `ungroundable` and only prove the ceilings do *not* fire when they should not. That "no repository of record" rule is load-bearing rather than incidental: without it, an auditor would ground the fixtures against whatever repo is open, find none of their fictional endpoints, and report `drifted` on all eight, dropping `excellent-task.md` off its expected Excellent and failing the suite for a reason unrelated to the Scorecard
...
- `/bead-audit-all` - Single-pass, report-only audit of the whole backlog: enumerate every bead via `br`, score and ground each once, report a ranked health table (worst band first). Bounded, not a `/goal` loop. **Reads** `skills/bead-audit/SKILL.md` from disk rather than invoking it through the Skill tool, for the same reason `/response-style` does: `commands/<name>.md` and `skills/<name>/SKILL.md` share one `tadw:` invocation namespace, and the command wins, so `Skill(bead-audit)` returns the twenty-line command body ("Use the `bead-audit` skill") which redirects to itself and never reaches the 661-line rubric. The audit would then score from a summary and report confident numbers computed from nothing, which is why step 4 also asserts three headings must be present in the file it read. **18 commands currently collide with a same-named skill** (`plan-review`, `verify-acceptance`, `plan-to-beads`, `product-surface-docs`, `roadmap-dashboard`, and the product-strategy set among them); the collision is only a live bug where the command's body delegates with "load the skill" instead of carrying its own instructions, and only `/bead-audit-all` has been fixed so far
...
- **`SessionStart`** injects the core plus the response style, sourced from the
  ...
  licensed dictionary), using American English, and report your own work in a fixed shape,
  never letting a label like "green" or "a flake" stand without the facts it stands for;
  put multi-factor
