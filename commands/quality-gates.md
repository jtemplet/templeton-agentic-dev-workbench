...
9. Write that verdict, the routing, and every gate row to `quality-gates-report.json` inside the git
   directory, so a tool can read the conclusion instead of parsing prose. That is
...
the working tree to establish a baseline. The single file it writes is the JSON artifact above,
which sits inside the git directory, is never committed, and is skipped entirely when the tree is
