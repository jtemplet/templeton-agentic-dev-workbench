...
| `reconcile-acceptance` | Fix the FAIL criteria and FAIL gates a NOT ACCEPTED `/tadw:verify-acceptance` run recorded in `acceptance-report.json`, then stop without re-grading the fix: reads findings only from that file, refuses a missing, stale, foreign, or unreadable report, runs each finding's `command` before committing when it names one, refuses a file with uncommitted changes, and ends with one `RECONCILE_ACCEPTANCE_DONE <n>` / `RECONCILE_ACCEPTANCE_BLOCKED <reason>` line | After a NOT ACCEPTED verdict, or inside an unattended loop that calls it up to a small attempt cap |
| `reconcile-quality-gates` | Fix the FAIL gate findings a FAIL `/tadw:quality-gates` run recorded in `quality-gates-report.json`, then stop without re-grading the fix: reads findings only from that file, refuses a missing, stale, foreign, or unreadable report, runs each finding's gate `command` before committing, refuses a file with uncommitted changes, and ends with one `RECONCILE_QUALITY_GATES_DONE <n>` / `RECONCILE_QUALITY_GATES_BLOCKED <reason>` line | After a FAIL verdict, or inside an unattended loop that calls it up to a small attempt cap |
