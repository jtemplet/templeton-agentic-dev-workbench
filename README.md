...
| `verify-acceptance` | Grade a finished unit of work against its bead's `acceptance_criteria` and the QA gates; every criterion graded against a named test, a command's output, or a `file:line`, never the diff; report-only ACCEPTED / NOT ACCEPTED / INCONCLUSIVE | Deciding whether work is done, before `bd close` or a PR |
| `quality-gates` | QA the change rather than the repository: scoped to the diff by default, takes the gate list from `AGENTS.md`/CI/a task runner before guessing, and adds a change-coverage gate that enumerates the cases the diff introduces, requires a unit test for each plus an end-to-end test through the real entry point for every CLI command or HTTP route touched, and grades the span (input, boundary, state, outcome classes) while explicitly refusing cross-products and defensive code; browser UI is HANDOFF to `/qa`, an unrunnable gate is BLOCKED, and an all-skip run is not a pass; report-only | Ending a session, before a PR, or before `bd close` |
