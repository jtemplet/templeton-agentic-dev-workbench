...
| `triage-beads` | Rank the open `br` backlog by ROI (value ÷ effort) into a one-screen readout: one top pick with its scoring arithmetic and claim command, a scored leaderboard naming what each runner-up lost on, a blocked list naming each blocker, and a capped ROI-ordered tail; takes readiness from `br ready`/`br blocked`, the measured graph facts (unblock counts, PageRank) from `bv --robot-triage`, and prices each bead on a fixed evidence-cited rubric so the same tracker state always yields the same pick; report-only, `br`/`bv` CLI only, no MCP | Choosing the next bead to claim, or when `br ready` output has stopped being scannable |
